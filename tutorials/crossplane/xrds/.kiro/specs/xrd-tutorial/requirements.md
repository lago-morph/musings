...
1. THE ApiEndpoint_XRD SHALL encapsulate API Gateway REST API, Lambda function, and IAM role resources
...
4. THE ApiEndpoint_XRD SHALL include status fields for endpointUrl, deploymentTime, lambdaArn, invocationCount, and lastInvoked
...
7. THE Tutorial_System SHALL include real CloudWatch metrics in status fields that change independently of Crossplane
...
1. THE ApiRoute_XRD SHALL encapsulate API Gateway Method/Route and Integration resources
...
4. THE ApiRoute_XRD SHALL include status fields for routeStatus, createdAt, integrationId, requestCount, and avgResponseTime
5. WHEN status fields are populated, THE Composition_Function SHALL compute custom status aggregation
6. THE Composition_Function SHALL retrieve real CloudWatch metrics for requestCount and avgResponseTime
...
### Requirement 6: Real Metrics Integration for Status Fields
...
1. THE Tutorial_System SHALL integrate CloudWatch API calls for real metric retrieval
...
4. WHEN CloudWatch API calls fail, THE Composition_Function SHALL handle errors gracefully without failing composition
...
1. WHEN performance implications exist between approaches, THE Tutorial_System SHALL provide brief mentions with links to Crossplane documentation
...
1. THE Tutorial_System SHALL provide Layer 4 troubleshooting content in how-to guide format
...
7. THE Tutorial_System SHALL structure troubleshooting content for direct problem-solving use, not tutorial narrative
